...
4. Type in the name of the directory to store your project, e.g. "r-course". _You should avoid using spaces and fullstops in the project name_.
5. If you are using a teaching cluster machine, choose "Browse" and select `C:\work`  
6. Click the "Create Project" button.

(There may already be an `r-course` folder in `C:\work` from a previous presentation of the course.  Feel free to delete it if this is the case)
...
Note that the path ("c: > Work > r-course ...") will vary according to where you created the project.
...
> or when we use the `ls()` function (unless we use `ls(all.names = TRUE)`. The `ls()` function is introduced in the next episode.  Hidden variables are used for things
...
of typing errors).  Tab completion works in R whether you are using it from the command line or RStudio. 
 In RStudio this feature is even more useful; a
