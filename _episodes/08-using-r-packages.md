...
Before we cover installing packages, we should consider *where* the packages get installed.  `R` can look for packages in multiple locations, which can be set, or examined with the `.libPaths()` function (note the "." before the function name).  `R` is typically installed so that a user-specific directory is first searched for packages, followed by a system-wide directory.

By default, packages will be installed to the location that appears first in your `.libPaths()`.  This is the case on the University machines; packages you install will, by default, be placed on your `P:` drive.   The standard University image already has a number of packages pre-installed.   

You can add a new path to the search using `.libpaths("path/to/directory")`.  If you are unable to install packages on your machine you should add a path you have write access to.  
